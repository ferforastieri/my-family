...
export type LandingPlan = {
  interval: 'monthly' | 'semiannual' | 'annual' | 'lifetime';
  name: string;
  description: string;
  priceCents: number;
  currency: string;
...
type Copy = {
  ...
  privateSite: string;
  bestValue: string;
  period: Record<LandingPlan['interval'], string>;
};
...
export function renderLanding(
  ...
  const planCards = plans
    .map(
      (plan) =>
        `<article class="price-card${plan.highlighted ? ' highlighted' : ''}">${plan.highlighted ? `<span class="plan-badge">${escapeHtml(t.bestValue)}</span>` : ''}<h3>${escapeHtml(plan.name)}</h3><p>${escapeHtml(plan.description)}</p><div class="plan-price"><strong>${escapeHtml(formatMoney(plan.priceCents, plan.currency, locale))}</strong><span>${escapeHtml(t.period[plan.interval])}</span></div><a class="button ${plan.highlighted ? 'primary' : 'ghost'}" href="/app/signup?locale=${locale}&plan=${plan.interval}">${escapeHtml(t.primary)}</a></article>`,
    )
    .join('');
  ...
  return document({
    ...
    body: `
      ...
      <main>
        ...
        <section class="section"><div class="shell"><div class="section-heading"><h2>${escapeHtml(t.priceTitle)}</h2><p>${escapeHtml(t.priceBody)}</p></div>${planCards ? `<div class="price-grid">${planCards}</div>` : `<div class="price"><div><h2>${escapeHtml(t.priceTitle)}</h2><p>${escapeHtml(t.priceBody)}</p></div><a class="button" href="/app/signup?locale=${locale}">${escapeHtml(t.primary)} →</a></div>`}</div></section>
...
export function renderFamilySite(
  ...
  const socialPath =
    seo.socialImagePath ||
    brand.coverPath ||
    photos.find((photo: any) => photo?.url)?.url;
  const socialImage = socialPath
    ? mediaUrl(origin, slug, socialPath)
    : undefined;
  const sectionContent: Record<string, string> = {
    events: section(
      events.length > 0,
      `<div class="event-grid">${events.map((event: any) => `<article class="card"><div class="feature-icon">${escapeHtml(String(event.icon || '♥'))}</div><h3>${escapeHtml(String(event.title || ''))}</h3><div class="event-date">${formatDate(event.date, locale)}</div><p>${escapeHtml(String(event.message || ''))}</p></article>`).join('')}</div>`,
    ),
    gallery: contentSection(
      ...
      photos
        .map(
          (photo: any) =>
            `<article class="card memory">${photo.url ? `<img loading="lazy" alt="${escapeHtml(String(photo.texto || photo.album || t.memories))}" src="${escapeHtml(mediaUrl(origin, slug, photo.url))}">` : ''}<div class="memory-body"><h3>${escapeHtml(String(photo.album || t.memories))}</h3>${photo.texto ? `<p>${escapeHtml(String(photo.texto))}</p>` : ''}</div></article>`,
        )
        .join(''),
      'memory-grid',
    ),
    songs: contentSection(
      t.playlist,
      songs
        .map(
          (song: any) =>
            `<article class="card"><div class="feature-icon">♫</div><h3>${escapeHtml(String(song.titulo || ''))}</h3><p>${escapeHtml([song.artista, song.descricao].filter(Boolean).join(' · '))}</p></article>`,
        )
        .join(''),
    ),
    journey: contentSection(
      t.journey,
      journey
        .map(
          (item: any) =>
            `<article class="card"><div class="feature-icon">🌿</div><h3>${escapeHtml(String(item.titulo || ''))}</h3><p>${escapeHtml(String(item.conteudo || ''))}</p></article>`,
        )
        .join(''),
    ),
    letters: contentSection(
      t.letters,
      letters
        .map(
          (item: any) =>
  ...
  const visibleSections = Array.isArray(config.sections)
    ? [...config.sections]
        ...
        .map((item: any) => sectionContent[item.key] || '')
        .join('')
    : '';
  const primaryColor = safeColor(brand.primaryColor, '#ff69b4');
  const secondaryColor = safeColor(brand.secondaryColor, '#7c3aed');
  const cover = brand.coverPath
    ? ` style="background-image:linear-gradient(rgba(255,255,255,.78),rgba(255,255,255,.92)),url('${escapeHtml(mediaUrl(origin, slug, brand.coverPath))}')"`
    : '';
  const logo = brand.logoPath
    ? `<img class="site-logo" src="${escapeHtml(mediaUrl(origin, slug, brand.logoPath))}" alt="${escapeHtml(name)}">`
    : '🌸';
  const familyLoginPath = `/app/familia/${encodeURIComponent(slug)}/login?locale=${locale}`;

  return document({
    locale,
    title: String(seo.title || `${name} — ${t.brand}`),
    description: String(seo.description || t.privateSite),
    canonical,
    origin,
    robots: 'index, follow',
    socialImage,
    body: `
      <style>:root{--pink:${primaryColor};--pink-dark:${secondaryColor}}</style>
      <header class="nav"><div class="shell nav-inner"><a class="brand" href="/${locale}"><span class="brand-mark">${logo}</span><span>${escapeHtml(name)}</span></a><div class="nav-actions"><a class="button ghost" href="${familyLoginPath}">${escapeHtml(t.login)}</a><a class="button ghost" href="/${locale}">← ${escapeHtml(t.brand)}</a></div></div></header>
      <main>
        <section class="site-hero shell"${cover}><span class="eyebrow">♥ ${escapeHtml(t.eyebrow)}</span><h1>${escapeHtml(name)}</h1><p>${escapeHtml(String(seo.description || t.privateSite))}</p></section>
        ${visibleSections}
      </main>`,
  });
}
...
function nav(locale: MarketingLocale, t: Copy, origin: string) {
  return `<header class="nav"><div class="shell nav-inner"><a href="/${locale}" class="brand"><span class="brand-mark">🌸</span><span>${escapeHtml(t.brand)}</span></a><div class="nav-actions"><div class="locale-switch">${(
    ...
    .join(
      '',
    )}</div><a class="button ghost" href="/app/login/cliente">${escapeHtml(t.login)}</a><a class="button primary hide-mobile" href="/app/signup?locale=${locale}">${escapeHtml(t.primary)}</a></div></div></header>`;
}
...
function safeColor(value: unknown, defaultColor: string) {
