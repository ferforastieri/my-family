...
export class PublicSiteService {
  constructor(
    ...
    @InjectModel(CartaDocument.name)
    private letters: Model<CartaMongoDocument>,
  ) {}
    ...
    return this.context.run(
      ...
      async () => {
        const [home, photos, songs, letters, journey] = await Promise.all([
          ...
          this.photos.find().sort({ data: -1, createdAt: -1 }).limit(24).lean().exec(),
          this.songs.find().sort({ data: -1, createdAt: -1 }).limit(24).lean().exec(),
          this.letters
            .find({ tipo: 'letter' })
            .sort({ createdAt: -1 })
            .limit(12)
        ...
        return {
          ...
          letters: isDemo ? letters.map(stripDocument) : [],
          journey: isDemo ? journey.map(stripDocument) : [],
        };
      },
    );
  }
