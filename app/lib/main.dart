...
void main() {
  ...
  runApp(MyFamilyApp(auth: auth, theme: theme, repository: FamilyRepository(socket)));
  auth.bootstrap();
  theme.bootstrap();
}

class MyFamilyApp extends StatelessWidget {
  const MyFamilyApp({super.key, required this.auth, required this.theme, required this.repository});
    ...
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        return MaterialApp.router(
          ...
          debugShowCheckedModeBanner: false,
          theme: buildAppTheme(color: theme.color, mode: theme.mode),
          routerConfig: buildRouter(auth, theme, repository),
        );
      },
    );
  }
