...
const inputClass = "w-full p-4 border-2 border-input rounded-xl font-[Dancing_Script] text-lg transition-all bg-background focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 placeholder:text-muted-foreground";
const momentos = [
  'Primeiro Encontro',
  'Primeiro Beijo',
  'Pedido de Namoro',
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-[1000] p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-gradient-to-b from-[var(--love-bg-start)] to-card dark:to-card p-8 rounded-2xl w-[90%] max-w-[600px] shadow-xl animate-slide-in" onClick={e => e.stopPropagation()}>
        <h2 className="text-primary font-[Pacifico] text-2xl text-center mb-6">Adicionar Nova Música</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex gap-4 justify-end mt-4">
            <button type="button" onClick={onClose} className="px-8 py-3 rounded-[25px] font-[Dancing_Script] text-lg cursor-pointer transition-all hover:-translate-y-0.5 bg-transparent text-primary border-2 border-primary hover:bg-accent">
              Cancelar
            </button>
            <button type="submit" className="px-8 py-3 rounded-[25px] font-[Dancing_Script] text-lg cursor-pointer transition-all hover:-translate-y-0.5 bg-primary text-primary-foreground border-0 shadow-md hover:opacity-90">
              Adicionar Música
            </button>
          </div>
        </form>
